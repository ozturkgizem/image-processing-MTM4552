...
void histogramEqualization(IMAGE *image)
{
	...
	h=image->bmpih.bih;
	rowsize=(image->bmpih.bibitcount*w+31)/32*4;
	histogram(image,CDF);//calculates Cumulative Distrubition Function (Step 2)
	for(i=0;i<256;i++) Normalization[i]=255*CDF[i];//Step 3
	//Step 4: Construct new image
	for(i=0;i<h;i++)
	  for(j=0;j<rowsize;j++)
...
int Menu()
{
	...
	printf("4.Mean Filter\n");
	printf("5.Exit\n");
	printf("Select 1-5:");
	scanf("%d",&ch);
	return ch;
}
BYTE findMin(BYTE **W,int wsize)
{
	int i,j,min;
	min=W[0][0];
	for(i=0;i<wsize;i++)
	   for(j=0;j<wsize;j++) if(W[i][j]<min) min=W[i][j];
	return min;
}
BYTE findMax(BYTE **W,int wsize)
{
	int i,j,max;
	max=W[0][0];
	for(i=0;i<wsize;i++)
	   for(j=0;j<wsize;j++) if(W[i][j]>max) max=W[i][j];
	return max;
}
void minFilter(IMAGE *image,int wsize)
{
	int h,w,rowsize,i,j,k,n,m,l;
	BYTE *data;
	BYTE **W=(BYTE**)malloc(wsize*sizeof(BYTE*));
	for(i=0;i<wsize;i++) W[i]=(BYTE*) malloc(wsize*(sizeof(BYTE)));
	w=image->bmpih.biw;
	h=image->bmpih.bih;
	rowsize=(image->bmpih.bibitcount*w+31)/32*4;
	...
	for(i=1;i<h-l;i++)
		for(j=1;j<rowsize-l;j++)
		{
			for(n=-l;n<=l;n++)
			   for(m=-l;m<=l;m++)
			      W[n+1][m+1]=data[(i+n)*rowsize+j+m];
			image->data[i*rowsize+j]=findMin(W,wsize);
     	}
    free(data);
    return;
}
void meanFilter(IMAGE *image,int wsize)
{
	int h,w,rowsize,i,j,k,n,m,l;
	BYTE *data;
	int T;
	...
	for(i=1;i<h-l;i++)
		for(j=1;j<rowsize-l;j++)
		{
			...
			for(n=-l;n<=l;n++)
			   for(m=-l;m<=l;m++)
			      T+=data[(i+n)*rowsize+j+m];
			image->data[i*rowsize+j]=(BYTE)(T/(wsize*wsize));
     	}
    free(data);
    return;
}
...
int findMedian(BYTE *W,int wsize)
{
	int i,j;
	int k;
	for(i=0;i<wsize;i++)
...
void medianFilter(IMAGE *image,int wsize)
{
	int h,w,rowsize,i,j,k,n,m,l;
	BYTE *data;
	BYTE *W=(BYTE*)malloc(wsize*wsize*sizeof(BYTE));
	w=image->bmpih.biw;
	h=image->bmpih.bih;
	rowsize=(image->bmpih.bibitcount*w+31)/32*4;
	data=(BYTE*)malloc(h*rowsize*sizeof(BYTE));
	...
	for(i=1;i<h-l;i++)
		for(j=1;j<rowsize-l;j++)
		{
			...
			for(n=-l;n<=l;n++)
			   for(m=-l;m<=l;m++)
			      W[k++]=data[(i+n)*rowsize+j+m];
			image->data[i*rowsize+j]=findMedian(W,wsize*wsize);
     	}
    free(data);
    return;
}
void maxFilter(IMAGE *image,int wsize)
{
	int h,w,rowsize,i,j,k,n,m,l;
	BYTE *data;
	BYTE **W=(BYTE**)malloc(wsize*sizeof(BYTE*));
	...
	for(i=1;i<h-l;i++)
		for(j=1;j<rowsize-l;j++)
		{
			for(n=-l;n<=l;n++)
			   for(m=-l;m<=l;m++)
			      W[n+1][m+1]=data[(i+n)*rowsize+j+m];
			image->data[i*rowsize+j]=findMax(W,wsize);
     	}
    free(data);
    return;
}
void lowPass(IMAGE *image,int wsize)
{
	int k;
    k=Menu();
	if(k==5) return;
	...
	{
		case 1:
			minFilter(image,wsize);
			break;
		case 2:
			maxFilter(image,wsize);
			break;
			
		case 3:
			medianFilter(image,wsize);
			break;
		case 4:
			meanFilter(image,wsize);
			break;
    }
...
int highPassMenu()
{
	...
	do
	{
		printf("1. Simple Laplacien Filte\n");
		printf("2. Variant of Laplacian\n");
		printf("3. Sobel X Filter\n");
		printf("4. Sobel Y Filter\n");
		printf("5. Exit\n");
		printf("Select 1-5:");
		k=scanf("%d",&choice);
		if(k<1 || choice<1 || choice>5) printf("Wrong selction. Enter an itegeger of 1-5");
		
	}while(k<1 || choice<1 || choice>5);
	return choice;
...
main()
{
	...
	image=ImageRead(image,"biber.bmp");//reads the image from file and sets 1all info to the ram
	highPass(image);
	
    ImageWrite(image,"highpassfilter.bmp");
    free(image);	
    system("pause");
	return 0;
}
