...
main()
{
	...
	image=ImageRead(image,"kelebek3.bmp");//reads the image from file and sets 1all info to the ram
	
    writeInfo(image,"kelebek.bmp");
   // brightImage(image,100);
	//----------------------------
    histogram(image,"kelebek3.txt");
    maxAndMinIntensity(image,max,min);
    printf("Maximum pixel value :%d\n",max);
    printf("Minimum pixel value :%d\n",min);
    printf("Constrast value:%d\n",max-min);
//	ImageWrite(image,"kelebek3.bmp");//writes this image to file named "test3.bmp"
